...
def get_clang_cmds(compile_cmd):
    ...
    ll_args = args + "-Xclang -disable-O0-optnone -g -S -O0 -fno-inline -emit-llvm -fno-discard-value-names -o".split()
    compile_dir = get_compile_dir(compile_cmd)
    return [
        [
            "echo -e '\n' Building " + cache_base_name,
            "cd " + get_compile_dir(compile_cmd),
            proc_args[0] + " " + shlex.join(proc_args[1:]) + f" 2> {ast_out_dir}/{stderr_file} | gzip > {ast_out_dir}/{cache_ast_file}; echo $? > {ast_out_dir}/{retcode_file}",
            ll_args[0] + " " + shlex.join(ll_args[1:]) + " " + ast_out_dir + "/" + ll_raw_file
        ],

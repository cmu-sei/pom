...
The following command, when run in the POM container, creates a compilation database file (named `compile_commands.json`) for dos2unix:
...
This creates the `/host/d2u/run_clang.sh` file.  This is a shell script with several Clang commands.

For this step, and all the following steps, any new file in the `d2u` folder should have an identical file in the `d2u.good` folder with the same contents. (Note that if you solicit new responses from the GPT, those responses may differ from ours.) 0You can confirm this with a diff command:

```sh
diff -r /host/d2u /host/d2u.good
```

While `d2u.good` will have files that `d2u` currently lacks, all files they have in common should be identical.
...
```sh
bash /host/d2u/run_clang.sh /host/d2u
```

This creates the various `*.raw.*` files.  These are the ASTs and LLVM IRs from the source files in dos2unix.

### Running the LLM-based p-model Builder

You now have the AST and source code. With those as input, you can run the LLM-based Builder. To do this, you must have the `OPENAI_API_KEY` environment variable set up as per OpenAI's instructions for accessing GPT from the command line, and your machine must be able to access GPT. (For an example of how to provide your `OPENAI_API_KEY` to the container, see `docker run` instruction including `secret-tool lookup` in the [Build instructions](#build-instructions) section.)

If you have more than 3 files that match `/host/d2u/*.ast.json.gz`, delete all but the newest 3.

If you do not wish to use GPT, but instead rely on our cached responses from GPT, then execute this command first. If you wish to use GPT, then you can skip this command:

```sh
cp -r /host/d2u.good/cache /host/src
```

The command to run the builder is:

```sh
cd /host/src
python3 build.py --verbose /host/d2u/dos2unix.llm.pom.yml /datasets/dos2unix-7.5.2  /host/d2u/*.ast.json.gz
```

This will produce a `/host/d2u/dos2unix.llm.pom.yml`, the p-model constructed by GPT.  Unless they are already present, it will also produce many `*.query` and `*.reply` files in `/host/src/cache`. These files represent GPT prompts and GPT's replies.
...
First, perform the quick "Demo of basic use" [here](src/constraint_gen/README.sat.solver.md). That produces a `solution.json` file in directory `src/constraint_gen/out`.
...
The model it creates is slightly different but compatible with the manually-generated [p-model for test/swap.c](./src/test/swap.pom.yml).

### Verifying Builder Output

The following files can be used to verify that the builder's p-model is correct.

 * `/host/d2u.good/dos2unix.manual.pom.yml`

A model created by hand while studying the dos2unix source code

 * `/host/d2u.good/dos2unix.sorted.pom.yml`
...
```sh
diff -u /host/d2u/dos2unix.llm.pom.yml /host/d2u.good/dos2unix.sorted.pom.yml
```

If there are no differences from the LLM-produced p-model, then the LLM was 100% accurate in generating the model. (The accuracy was not quite 100% in the cached GPT responses).
...
We use two p-models for dos2unix (one with library functions used by dos2unix: [dos2unix.famous.manual.pom.yml](./d2u/dos2unix.famous.manual.pom.yml), the other is [dos2unix.llm.pom.yml](./src/dos2unix.llm.pom.yml). We use whole-program mode for this example.

```
export LLVM_OPTS="-whole-prog"
cd /host/src/constraint_gen
POM_FILE="/host/d2u.good/dos2unix.llm.pom.yml /host/d2u.good/dos2unix.famous.manual.pom.yml" ./run.sh /host/d2u/combined.raw.ll out
```

The output should say `UNSATISFIABLE`, which is also the result if you use our manually-built file `dos2unix.sorted.pom.yml`:

`POM_FILE="/host/d2u.good/dos2unix.sorted.pom.yml /host/d2u.good/dos2unix.famous.manual.pom.yml"  ./run.sh /host/d2u/combined.raw.ll out`
...
More detail about our SAT-solver verifier, along with demonstration examples with detailed steps (one `SAT`, one `UNSAT`, one using an input p-model file, and a demo of summarizing a function), commands, and explanation of outputs (plus more) is provided [here](src/constraint_gen/README.sat.solver.md).
...
``` sh
cd /host
python3 src/verify.py d2u/dos2unix.llm.pom.yml d2u/common.*.raw.ast.json.gz > verifier.output
```
...
This project uses externally-developed open-source software. Most of it is specified in and installed by the [Dockerfile](./Dockerfile), except for ghostq, located in [src/constraint_gen/ghostq](src/constraint_gen/ghostq). Such software includes (among others):

| Software           | Version     | Host   | License                |
| [bear](https://github.com/rizsotto/Bear)          | 3.1.3       | Github | GPL 3 
